...
function () {
  ...
  function Week(_epoch) {
    ...
    this.baseClass = new BaseClass$2(_epoch); //var date = 1; 		var month = 1; 		var year = 2019;

    var date = this.baseClass.epoch.date;
    var month = this.baseClass.epoch.month;
    var year = this.baseClass.epoch.year;
    var dayNumber = this.baseClass.getDayNumber(month, date);

    var day = GregorianDay(year, month, date);
    var daysInMonth = this.baseClass.getDaysInMonth(BaseClass$2.LOOKUPTABLE[month - 1]); //console.log(`${_epoch.toLocaleString()} is a ${"Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Saturday".split(",")[day]} Day = ${day} Date = ${date}) Week Number = ${weekNo} Day Number = ${this.baseClass.getDayNumber(month, date)}`);
